...
class TriggerableManager:
    ...
    def get_items_sorted(self):
        ...
        # systems.utils.debug_print('items sorted')
        # systems.utils.debug_print(sorted_items)
...
class InventoryManager:
    ...
    def get_item_by_premade_id(self, item_id):
        for i in self.items.values():
            # systems.utils.debug_print(i.premade_id)
    ...
    def remove_items_by_id(self, item_premade_id, stack=0):
        ...
        if type(self.owner).__name__ == "Player":
            self.owner.quest_manager.propose_objective_count_addition(
                ObjectiveCountProposal(
                    OBJECTIVE_TYPES.COLLECT_X, item_premade_id, -stacks_removed
                )
            )
        ...
        if stack >= 1:
            if stack <= self.items[item.id].stack:
                if type(self.owner).__name__ == "Player":
                    self.owner.quest_manager.propose_objective_count_addition(
                        ObjectiveCountProposal(
                            OBJECTIVE_TYPES.COLLECT_X, item.premade_id, stack
                        )
                    )
