...
def calculate_skill_hp_cost(actor, base_value):
    _cost = int(base_value + (actor.stat_manager.stats[StatType.LVL] * 0.5))
    return _cost
...
def unload_fr():
    ...
    debug_print(f'REFS     : {len(REFTRACKER.refs)}')
...
def get_matches(line, things):
    ...
    i = 1
    for val in things.values():
        # systems.utils.debug_print(index, matches[index-1][0].lower(),'----------', i, val.name)
...
def get_match(line, things):
    ...
    for thing in things.values():
        ...
        thing_names.append(remove_color(thing.pretty_name()))
        # systems.utils.debug_print(thing_names)
...
def chunkate(input_string, max_width=800*3):
    ...
    output = ""
    line_length = 0
    # systems.utils.debug_print(chunks)
    for chunk in chunks:
        # systems.utils.debug_print(repr(chunk))
        ...
        if chunk == "\n":
            # systems.utils.debug_print('RESET CHUNJ')
        ...
        if line_length + word_length > max_width:
            ...
            # systems.utils.debug_print('NEW LINE')
...
def progress_bar(size, cur_value, max_value, color="@white", style=0):
    ...
    # systems.utils.debug_print(len(output))
    output = output
...
for i in range(0,101):
    ...
    systems.utils.debug_print(add_color(progress_bar(8,i,100,col)))
"""
