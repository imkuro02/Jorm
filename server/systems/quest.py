...
class QuestManager:
    ...
    def get_quest_id_from_quest_name(self, quest_name):
        ...
        quest_name = systems.utils.match_word(
            quest_name, [quest.name for quest in self.quests.values()]
        )
    ...
    def turn_in_quest(self, quest_id):
        ...
        proposal = ObjectiveCountProposal(
            OBJECTIVE_TYPES.TURNED_IN, OBJECTIVE_TYPES.TURNED_IN, 1
        )
        proposal_accepted = self.quests[quest_id].propose_objective_count_addition(
            proposal
        )
        ...
        if proposal_accepted:
            ...
            for objective in self.quests[quest_id].objectives.values():
                ...
                removed = self.actor.inventory_manager.remove_items_by_id(
                    objective.requirement_id, stacks_to_remove
                )
                ...
                if not removed:
                    ...
                    proposal_accepted = self.quests[
                        quest_id
                    ].propose_objective_count_addition(proposal)
                    return False
    ...
    def propose_objective_count_addition(
        self, objective_count_proposal: "ObjectiveCountProposal"
    ):
        proposal_accepted = False
        for quest in self.quests.values():
            if quest.objectives[OBJECTIVE_TYPES.TURNED_IN].is_completed():
                continue
            proposal_accepted = quest.propose_objective_count_addition(
                objective_count_proposal
            )
            if proposal_accepted:
                break
...
class Quest:
    ...
    def view(self):
        ...
        if self.is_turned_in():
            ...
            output += (
                f"{Color.DESC_QUEST}"
                + self.description["turned_in"]
                + f"{Color.NORMAL}\n"
            )

        for objective in self.objectives.values():
            # systems.utils.debug_print(objective.__dict__)
            if objective.type == OBJECTIVE_TYPES.TURNED_IN:
                continue

            if not self.objectives[OBJECTIVE_TYPES.TURNED_IN].is_completed():
                if (
                    not objective.hidden_objective_until_completed
                    or objective.count >= objective.goal
                ):
                    match objective.type:
                        case OBJECTIVE_TYPES.KILL_X:
                            enemy_name = ENEMIES[objective.requirement_id]["name"]
                            output += (
                                f"{objective.name}; Kill {Color.IMPORTANT}{enemy_name}{Color.NORMAL}: {objective.count}/{objective.goal}"
                                + "\n"
                            )
                        case OBJECTIVE_TYPES.COLLECT_X:
                            enemy_name = ITEMS[objective.requirement_id]["name"]
                            output += (
                                f"{objective.name}; Collect {Color.IMPORTANT}{enemy_name}{Color.NORMAL}: {objective.count}/{objective.goal}"
                                + "\n"
                            )
                        case OBJECTIVE_TYPES.CONVERSATION:
                            # npc = NPCS[objective.requirement_id]['name']
                            output += (
                                f"{objective.name}: {f'{Color.GOOD}Completed{Color.NORMAL}' if objective.count >= objective.goal else f'{Color.IMPORTANT}In Progress{Color.NORMAL}'}"
                                + "\n"
                            )
                else:
                    pass
                    output += f"{objective.name}; Uknown Objective\n"
        output += "\nQuest state: " + self.get_state(as_string=True) + "\n"
        return output
    ...
    def propose_objective_count_addition(
        ...
        for objective in self.objectives.values():
            ...
            proposal_accepted = objective.propose_objective_count_addition(
                objective_count_proposal
            )
            if (
                completed == False
                and objective.is_completed()
                and objective.type != OBJECTIVE_TYPES.TURNED_IN
            ):
...
class ObjectiveCountProposal:
    def __init__(self, _type, _requirement_id, _to_add=1):
...
class Objective:
    def __init__(
        ...
        _requirement_id,
        _goal,
        _hidden_objective_until_completed=False,
    ):
        ...
        self.hidden_objective_until_completed = _hidden_objective_until_completed
    ...
    def propose_objective_count_addition(
        ...
        # systems.utils.debug_print(self.__dict__, objective_count_proposal.__dict__)
...
def create_daily_quest(quest_id, actor):
    ...
    def create_objective(player_level):
        possible_enemies = [
            ...
            if ENEMIES[enemy]["stats"]["lvl"] <= player_level + 3
            and ENEMIES[enemy]["include_in_daily_quests"]
        ]
...
def create_quest(quest_id, actor):
    ...
    if quest_id not in QUESTS:
        systems.utils.debug_print(
            quest_id, "does not exist in configuration.config QUESTS"
        )
        return
...
if __name__ == "__main__":
    ...
    systems.utils.debug_print(quest.quest_id)
    systems.utils.debug_print(quest.objectives)
    for obj in quest.objectives:
        systems.utils.debug_print(obj, quest.objectives)
        systems.utils.debug_print(quest.objectives[obj].__dict__)
