...
class Dialog:
        ...
        for option in self.dialog_tree[self.current_line]["options"]:
            ...
            if "quest_check" in option:
                ...
                for check in option["quest_check"]:
                    ...
                    if not "NOT" in check:
                        ...
                        if (
                            self.player.quest_manager.check_quest_state(quest_id)
                            == quest_state
                        ):
                            this_option_is_good = False
            ...
            if "quest_objective_check" in option:
                ...
                for check in option["quest_objective_check"]:
                    ...
                    if (
                        quest_objective_id
                        not in self.player.quest_manager.quests[quest_id].objectives
                    ):
                        this_option_is_good = False
                        break

                    if (
                        self.player.quest_manager.quests[quest_id]
                        .objectives[quest_objective_id]
                        .count
            ...
            if "quest_turn_in" in option:
                ...
                if "reward_practice_points" in option["quest_turn_in"]:
                    dic["reward_practice_points"] = option["quest_turn_in"][
            ...
            if "quest_objective" in option:
                ...
                objective_count_proposal = ObjectiveCountProposal(
    ...
    def answer(self, line):
        ...
        if "line" in _line:
            output_line = "> " + _line["line"].strip("\n") + Color.NORMAL
        if "line_room_party" in _line:
            output_line_room_party = (
                "> " + _line["line_room_party"].strip("\n") + Color.NORMAL
            )
        if "line_room_not_party" in _line:
            output_line_room_not_party = (
                "> " + _line["line_room_not_party"].strip("\n") + Color.NORMAL
            )
        ...
        if "trade" in answer:
            ...
            if "give_to_player" in answer["trade"]:
                if self.player.inventory_manager.get_amount_of_free_item_slots() < len(
                    answer["trade"]["give_to_player"]
                ):
                    escape = True
            if "give_to_npc" in answer["trade"]:
                for i in answer["trade"]["give_to_npc"]:
                    if (
                        self.player.inventory_manager.get_stacks_of_premade_id(
                            i["item"]
                        )
                        < i["amount"]
                    ):
            ...
            if "give_to_npc" in answer["trade"]:
                for i in answer["trade"]["give_to_npc"]:
                    removed = self.player.inventory_manager.remove_items_by_id(
                        i["item"], i["amount"]
                    )
                    if not removed:
