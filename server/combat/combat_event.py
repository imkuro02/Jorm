...
class CombatEvent:
    ...
    def add_to_print(self, obj, diff):
        ...
        pop = {'id': f'{obj.damage_source_actor.id}-{obj.damage_taker_actor.id}', 'diff': diff, 'obj': obj}
    ...
    def run(self):
        ...
        if not damage_obj.dont_proc:
            ...
            if damage_obj.damage_source_actor:
                if damage_obj.damage_source_actor.affect_manager:
                    damage_obj = damage_obj.damage_source_actor.affect_manager.deal_damage(damage_obj)
                if damage_obj.damage_source_actor.inventory_manager:
                    damage_obj = damage_obj.damage_source_actor.inventory_manager.deal_damage(damage_obj)
                if damage_obj.damage_source_actor.skill_manager:
                    damage_obj = damage_obj.damage_source_actor.skill_manager.deal_damage(damage_obj)

            # before calc on damage_taker_actor
            if damage_obj.damage_taker_actor:
                if damage_obj.damage_taker_actor.affect_manager:
                    damage_obj = damage_obj.damage_taker_actor.affect_manager.take_damage_before_calc(damage_obj)
                if damage_obj.damage_taker_actor.inventory_manager:
                    damage_obj = damage_obj.damage_taker_actor.inventory_manager.take_damage_before_calc(damage_obj)
                if damage_obj.damage_taker_actor.skill_manager:
                    damage_obj = damage_obj.damage_taker_actor.skill_manager.take_damage_before_calc(damage_obj)
        ...
        if not damage_obj.dont_proc:
            ...
            if damage_obj.damage_taker_actor:
                ...
                if damage_obj.damage_taker_actor.skill_manager:
                    damage_obj = damage_obj.damage_taker_actor.skill_manager.take_damage_after_calc(damage_obj)
        
            # after calc on damage_source_actor
            if damage_obj.damage_source_actor:
                if damage_obj.damage_source_actor.affect_manager:
                    damage_obj = damage_obj.damage_source_actor.affect_manager.dealt_damage(damage_obj)
                if damage_obj.damage_source_actor.inventory_manager:
                    damage_obj = damage_obj.damage_source_actor.inventory_manager.dealt_damage(damage_obj)
                if damage_obj.damage_source_actor.skill_manager:
                    damage_obj = damage_obj.damage_source_actor.skill_manager.dealt_damage(damage_obj)
