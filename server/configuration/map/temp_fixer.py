...
with open(filename, "r") as file:
    ...
    for i in data["nodes"]:
        ...
        for e in data["edges"]:
            if i == data["edges"][e]["data"]["target"]:
                data["edges"][e]["data"]["target"] = data["edges"][e]["data"][
                    "target"
                ].replace(i, new_id)
            if i == data["edges"][e]["data"]["source"]:
                data["edges"][e]["data"]["source"] = data["edges"][e]["data"][
                    "source"
                ].replace(i, new_id)
    ...
    with open("output.json", "w") as outfile:
        json.dump(
            data, outfile, indent=4
        )  # `indent=4` makes it pretty-systems.utils.debug_printed


# systems.utils.debug_print the loaded data
# systems.utils.debug_print(data)
