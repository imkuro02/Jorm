    ...
    def view(self):
        ...
        for objective in self.objectives.values():
            ...
            if not self.objectives[OBJECTIVE_TYPES.TURNED_IN].is_completed():
                if not objective.hidden_objective_until_completed or objective.count >= objective.goal:
                    match objective.type:
                        case OBJECTIVE_TYPES.KILL_X:
                            enemy_name = ENEMIES[objective.requirement_id]['name']
                            output += f'{objective.name}; Kill {Color.IMPORTANT}{enemy_name}{Color.NORMAL}: {objective.count}/{objective.goal}' + '\n'
                        case OBJECTIVE_TYPES.COLLECT_X:
                            enemy_name = ITEMS[objective.requirement_id]['name']
                            output += f'{objective.name}; Collect {Color.IMPORTANT}{enemy_name}{Color.NORMAL}: {objective.count}/{objective.goal}' + '\n'
                        case OBJECTIVE_TYPES.CONVERSATION:
                            #npc = NPCS[objective.requirement_id]['name']
                            output += f'{objective.name}: {f"{Color.GOOD}Completed{Color.NORMAL}" if objective.count >= objective.goal else f"{Color.IMPORTANT}In Progress{Color.NORMAL}"}' + '\n'
                else:
                    pass
...
def create_daily_quest(quest_id, actor):
    ...
    quest.add_objective( Objective(quest_id, OBJECTIVE_TYPES.TURNED_IN, OBJECTIVE_TYPES.TURNED_IN, OBJECTIVE_TYPES.TURNED_IN, 1, False) )
    return quest
    ...
    for objective_name in quest_dict['objectives']:
        ...
        quest.add_objective(
            Objective(
                ...
                objective['objective'],
                objective['completed_at'],
                bool(objective['hidden_objective_until_completed']) if 'hidden_objective_until_completed' in objective else False
            )
        )
