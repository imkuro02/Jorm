...
def skill_checks(user, target, skill_id):
    ...
    if skill_id in user.skill_manager.cooldowns:
...
def check_if_skill_can_be_used(skill_object):
    ...
    if skill_id in user.skill_manager.charges:
        if user.skill_manager.charges[skill_id] >= int(skill_object.calculate_script_value(value = 'charges')):
...
def construct_skill(skill_id = None):
    try:
        ...
        script_to_run = f"Skill{SKILLS[skill_id]['script_to_run']}"
        systems.utils.debug_print(systems.utils.add_color(f'@redscript_to_run:{script_to_run} is not a valid skill object in skills.py@normal'))
        return None