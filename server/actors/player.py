        ...
        for r in _grid:
            ...
            for actor in room.actors.values():
                important_dialog_dict = actor.get_important_dialog(
                    actor_to_compare=self.actor, return_dict=True
                )
...
class Player(Actor):
    ...
    def gain_exp(self, exp):
        ...
        if exp < 0:
            self.send_line(
                f"{self.pretty_name(identifier=self)} lost {Color.BAD}" + str(abs(exp)) + f" experience{Color.BACK}"
            )
        if exp > 0:
            self.send_line(
