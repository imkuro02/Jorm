...
def raise_item(self, item_id):
    ...
    if item_id in self.inventory_manager.items:
        ...
        self.inventory_manager.items = {
            item_id: value,
            **self.inventory_manager.items,
        }  # reconstruct with the item first
...
def command_craft(self, line):
    ...
    if ingredients == []:
        # systems.utils.debug_print('if statement')
        # systems.utils.debug_print(item_crafting_recipes)
    ...
    if ingredients != []:
        for recipe in item_crafting_recipes:
            ...
            # systems.utils.debug_print(ingredients_to_use, len(ingredients_to_use))
            # systems.utils.debug_print(recipe, len(recipe))
    ...
    for index in range(0, len(recipe_to_use)):
        output = (
            output
            + f"{list(recipe_to_use.values())[index]} {ingredients_to_use[index].name}, "
        )
        # systems.utils.debug_print(':::', ingredients_to_use[index], list(recipe_to_use.values())[index])
        self.inventory_manager.remove_item(
            item=ingredients_to_use[index], stack=list(recipe_to_use.values())[index]
        )

    item = items.load_item(item_id)
    # systems.utils.debug_print(item.premade_id)
