...
def command_map3(self, line, return_gmcp=False):
        ...
        for room_loc in _grid:
            ...
            for _exit in room.get_active_exits(self):
                ...
                if not room.doorway:  # and room.get_real_id() in self.explored_rooms:
                    # systems.utils.debug_print(_exit.to_room_id, grid.values())
    ...
    # systems.utils.debug_print(grid)
    for _x in range(0, (VIEW_RANGE * 2) + 1):
        for _y in range(0, (VIEW_RANGE * 2) + 1):
                    ...
                    for d in directions.values():
                        if d in grid:
                            if grid[d] in self.room.world.rooms:
                                wall_data = self.room.world.rooms[
                                    grid[d]
                                ].get_wall_data()
                                if len(wall_data) == 3:
            ...
            for actor in room.actors.values():
                important_dialog_dict = actor.get_important_dialog(
                    actor_to_compare=self, return_dict=True
                )
            ...
            # systems.utils.debug_print(room)
            # left
            cell = ""
            # if room.doorway:
            #    cell += Art.DOOR_L
    ...
    for i in split_output[:-1]:
        ...
        while x < len(i):
            if i[x] == ' ':
                ...
                break
        #systems.utils.debug_print(strip_len, left_strip_len)
        if strip_len < left_strip_len:
            left_strip_len = strip_len
            #systems.utils.debug_print(i[x-10:])
    ...
    def pad_lines_to_be_map_width(combined_output):
        ...
        first_line_length = len(systems.utils.remove_color(lines[0]))
        # systems.utils.debug_print(first_line_length, f'"{(systems.utils.remove_color(lines[0]))}"')
        ...
        if first_line_length < MAP_WIDTH:
            ...
            lines = [
                (f" " * math.ceil(padding_needed / 2))
                + line
                + (f" " * math.floor(padding_needed / 2))
                for line in lines
            ]
