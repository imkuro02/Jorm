...
def inventory_equip(self, item, forced=False):
    if item.slot != None:
        ...
        if self.slots_manager.slots[item.slot] != None:
            self.inventory_unequip(
                self.inventory_manager.items[self.slots_manager.slots[item.slot]]
            )
        ...
        for stat_name in item.stat_manager.stats:
            ...
            # systems.utils.debug_print(stat_name, item.stat_manager.stats[stat_name])
