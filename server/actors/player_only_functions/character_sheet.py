...
def command_level_up(self, stat):
    ...
    self.sendLine(f'{Color.GOOD}{stat} {self.stat_manager.stats[stat]-increase_by} -> {self.stat_manager.stats[stat]}. {Color.NORMAL}')
                ...
                if val_nam in skill['script_values']:
                    ...
                    for val in skill['script_values'][val_nam]:
                        ...
                        if isinstance(val, int):
                            if val_nam == 'hp_cost':
                                val = utils.calculate_skill_hp_cost(actor = self, base_value = skill['script_values']['hp_cost'][index])
                            pass
                        #if isinstance(val, int):
                        #    if val_nam == 'mp_cost':
